...
# 🔥 PRO STREAMING ENGINE (FIXED FOR EXOPLAYER & DOWNLOAD MANAGER)
@routes.get('/stream/{msg_id}')
async def stream_video(request):
    msg_id = int(request.match_info['msg_id'])
    # Player ne Range maangi hai ya nahi, check karna
    ...
    try:
        ...
        status_code = 200 # Default Status (Normal Download ke liye)
        ...
        if range_header:
            ...
            status_code = 206 # Partial Status (Video aage bhagane ke liye)
        ...
        # Agar player ne specific part maanga hai tabhi Range Header bhejna hai
        if status_code == 206:
        ...
        # File bhejna shuru karo
        async for chunk in app.stream_media(message, offset=start, limit=content_length):
            try:
                await response.write(chunk)
            except Exception:
                # Agar ExoPlayer ne data lena band kar diya (Disconnect), toh crash se bachne ke liye chup chap ruk jao
                break
